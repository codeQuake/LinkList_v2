...
class LinksTagCloudCacheBuilder extends TagCloudCacheBuilder {
	...
	protected function rebuild(array $parameters) {
		...
		if (isset($parameters['languageIDs']) && !empty($parameters['languageIDs'])) {
			$conditions->add("tag_to_object.languageID IN (?)", array($parameters['languageIDs']));
		}
		$conditions->add("tag_to_object.objectTypeID = ?", array($objectType->objectTypeID));
		$conditions->add("tag_to_object.objectID = thread.threadID");
		$conditions->add("link.categoryID = ?", array($parameters['categoryID']));
		...
		if (!empty($tagIDs)) {
			$conditions = new PreparedStatementConditionBuilder();
			$conditions->add("tagID IN (?)", array(array_keys($tagIDs)));
