...
class LinkAction extends AbstractDatabaseObjectAction implements IClipboardAction {
	...
	public function update() {
		...
		if (! empty ( $objectIDs ))
			SearchIndexManager::getInstance ()->delete ( 'de.codequake.linklist.link', $objectIDs );
		if (! empty ( $objectIDs ))
			SearchIndexManager::getInstance ()->add ( 'de.codequake.linklist.link', $object->linkID, $object->message, $object->subject, $object->time, $object->userID, $object->username, $object->languageID );
	}
	...
	public function delete() {
		...
		foreach ( $this->links as $link ) {
			$linkIDs [] = $link->linkID;
			if ($link->attachments != 0)
				$attachedLinkIDs [] = $link->linkID;
		...
		if (! empty ( $linkIDs ))
			SearchIndexManager::getInstance ()->delete ( 'de.codequake.linklist.link', $linkIDs );
