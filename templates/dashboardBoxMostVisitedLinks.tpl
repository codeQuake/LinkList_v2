 {hascontent}
	<ul class="sidebarBoxList">
		{content}
			{foreach from=$mostVisitedLinks item=link}
				<li class="box24">
					...
					<div class="sidebarBoxHeadline">
						<h3><a href="{link application='linklist' controller='Link' object=$link->getDecoratedObject()}{/link}" class="linklistLink" data-link-id="{@$link->linkID}" data-sort-order="DESC" title="{$link->subject}">{$link->subject}</a> <small>({#$link->visits} {lang}linklist.links.visits{/lang})</small></h3>
