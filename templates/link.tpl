...
<head>
	...
	<script data-relocate="true">
		...
		$(function() {
			...
			{if MODULE_LIKE && $__wcf->getSession()->getPermission('user.like.canViewLike') && LINKLIST_ENABLE_LIKES}
				new LINKLIST.Link.Like({if $__wcf->getUser()->userID && $__wcf->getSession()->getPermission('user.like.canLike')}1{else}0{/if}, {@LIKE_ENABLE_DISLIKE}, {@LIKE_SHOW_SUMMARY}, {@LIKE_ALLOW_FOR_OWN_CONTENT});
			{/if}
					...
					<div>
						<header class="messageHeader">
							<div class="messageHeadline">
								...
								<p>
									<span class="username">
										<a class="userLink" data-user-id="{$link->userID}" href="{link controller='User' object=$link->getUserProfile()}{/link}">{$link->username}</a>
									</span>
