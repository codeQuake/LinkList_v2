{if $__linklist->isActiveApplication() && $__searchAreaInitialized|empty}
	...
	{capture assign='__searchHiddenInputFields'}<input type="hidden" name="types[]" value="de.codequake.linklist.link" />{if $category|isset}<input type="hidden" name="categoryIDs[]" value="{@$category->categoryID}" />{/if}{/capture}
{/if}